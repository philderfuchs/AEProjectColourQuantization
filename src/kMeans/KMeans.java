...
public class KMeans implements ColorQuantizer {
	...
	public HashSet<Pixel> quantizeWithFilter(Histogram histogram,
		...
		ArrayList<HistogramWithMean> resultHistograms = this.kMeans(histogram, preFilterColorCount);
		...
		for(int i = 0; i < k; i++) {
			reducedColorPalette.add(resultHistograms.get(i).getMean());
		}
...
//			histogramsWithMeans.add(new HistogramWithMean(new Pixel(r, g, b, 1), new Histogram()));
//		}
